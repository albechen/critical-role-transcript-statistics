...
def mapping_names(all_ep_df, C1_C2):
    ...
    for names, name_list in zip(names, name_list):
        all_ep_df[names] = all_ep_df["name"].map(
            lambda y: 1 if any(word in name_list for word in y.split()) else 0
        )
...
def combine_name_list(named_mapped_df):
    ...
    for name in names:
        ...
        filtered_names_df = filtered_names_df[
            [
                "name",
                "speech",
                "count",
    ...
    no_assigned_person = no_assigned_person[
        ["name", "speech", "count", "episode", "total_time", "start_time", "end_time"]
    ]
