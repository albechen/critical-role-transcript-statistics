...
def group_by_person_episode(org_names_df):
    ...
    person_episode_group = person_episode_group.groupby(["person", "episode"]).agg(
        {"total_time": ["count", "sum"], "count": ["sum"]}
    )

    person_episode_group = person_episode_group.unstack(level=1, fill_value=0).stack()
    person_episode_group = (
        person_episode_group.unstack(level=-1, fill_value=0).stack().reset_index()
    )

    person_episode_group.columns = person_episode_group.columns.map("_".join).str.strip(
        "_"
    )
    person_episode_group["episode"] = person_episode_group["episode"].astype("int")
    person_episode_group = person_episode_group.rename(
        columns={
            "total_time_count": "line_count",
            "total_time_sum": "time_count",
...
def group_by_episode(org_names_df):
    ...
    person_group = (
        org_names_df.groupby(["episode"]).agg({"total_time": ["sum"]}).reset_index()
    )
...
def count_per_episode(person_episode_group):
    ...
    for name, w_count, l_count, t_count in zip(
        person_episode_group["person"],
...
def sentiment_per_person_and_episode(org_name):
    speech_per_person_ep = org_name.groupby(["episode", "person"])["speech"].apply(
        lambda x: ". ".join(x)
    )
    ...
    speech_per_person_ep["Subjectivity"] = speech_per_person_ep["speech"].apply(
        lambda x: TextBlob(x).sentiment.subjectivity
    )
    speech_per_person_ep["Polarity"] = speech_per_person_ep["speech"].apply(
