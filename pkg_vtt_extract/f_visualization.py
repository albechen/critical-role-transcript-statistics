...
def densityplot_time_per_ep2 (org_name, C1_C2):
    ...
    for name in names:
        ...
        if name != 'matt':
            ax0 = sns.kdeplot(person_episode_group["time_count"][(person_episode_group["person"] == name)], 
                    legend=False, shade=True, color=color, ax=ax[0])
            ax0.set_xlim(left=0)
            ax0.set_xlabel('Seconds per Episode')
            ax0.title.set_text('%s: Time Spoken Distribution' % C1_C2)

        ax1 = sns.kdeplot(person_episode_group["time_count"][(person_episode_group["person"] == name)], 
                legend=True, shade=True, color=color, ax=ax[1])
        ax1.legend(names)
        ax1.set_xlim(left=0)
...
def densityplot_time_per_ep (org_name, matt_nomatt, C1_C2):
    ...
    for name in names:
        color = palette[name]
        ax = sns.kdeplot(person_episode_group["time_count"][(person_episode_group["person"] == name)], 
...
def density_subjectivity2 (pol_sub_df, C1_C2):
    ...
    for name in names:
        ...
        ax0 = sns.kdeplot(pol_sub_df['Polarity'][(pol_sub_df["person"] == name)], 
                legend=False, shade=True, color=color, ax=ax[0])
        ax0.set_xlabel('Polarity')
        ax0.title.set_text('%s: Polarity Distribution' % C1_C2)

        ax1 = sns.kdeplot(pol_sub_df['Subjectivity'][(pol_sub_df["person"] == name)], 
                legend=True, shade=True, color=color, ax=ax[1])
        ax1.legend(names)
...
def density_subjectivity (pol_sub_df, C1_C2, Polarity_Subjectivity):
    ...
    for name in names:
        color = palette[name]
        ax = sns.kdeplot(pol_sub_df[Polarity_Subjectivity][(pol_sub_df["person"] == name)], 
                shade=True, color=color)
    ax.legend(names)
    ...
    path = 'images/density_sentiment' + '_' + C1_C2 + '_' + Polarity_Subjectivity + '.png'
    plt.savefig(path, dpi=500, bbox_inches = 'tight', pad_inches = 0.2)


def lineplot_sentiment(pol_sub_df, C1_C2, Polarity_Subjectivity):
    pol_sub_df = pol_sub_df.loc[(pol_sub_df['person'] != 'unassigned') & 
                                (pol_sub_df['person'] != 'guest')]
    palette ={'laura':'C0', 
        'matt':'C7', 
        'marisha':'C2', 
        'sam':'C3', 
    ...
    sns.set_style('whitegrid')
    plt.figure(figsize=(15,5))
    ax = sns.lineplot(x="episode", y=Polarity_Subjectivity, data=pol_sub_df, 
        hue='person', marker='.', palette=palette)
    ...
    path = 'images/lineplot_sentiment' + '_' + C1_C2 + '_' + Polarity_Subjectivity + '.png'
    plt.savefig(path, dpi=500, bbox_inches = 'tight', pad_inches = 0.2)